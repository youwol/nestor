...
timeout=30
jq_cmd='if .status == "py-youwol ok" then "py-youwol started and healthy" else error ("incorrect status from py-youwol") end'

# wait for py-youwol readiness
echo "::group::Trying at most $timeout times to call healtz endpoint, waiting 1 second between retries"
...
echo "::endgroup::"
echo "::error::Failed to contact py-youwol instance after 30 seconds"
